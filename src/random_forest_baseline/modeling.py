"""Model fitting, evaluation, and artifact export for the baseline."""
...
def build_pipeline(config: BaselineConfig) -> Pipeline:
    """Build the imputation + Random-Forest baseline pipeline."""
    return Pipeline(
        steps=[
...
def evaluate_grouped_dataset(dataset: PreparedDataset, config: BaselineConfig) -> EvaluationResult:
    """Evaluate the baseline by leaving one labeled recording session out per fold."""
...
def fit_final_model(dataset: PreparedDataset, config: BaselineConfig) -> tuple[Pipeline, pd.DataFrame]:
    """Fit the baseline pipeline on all currently selected labeled events."""
    pipeline = build_pipeline(config)
...
def save_confusion_matrix_plot(
    ...
    display.plot(ax=axis, colorbar=False, cmap="Blues", xticks_rotation=35, values_format="d")
    axis.set_title("Random-Forest Baseline")
    figure.tight_layout()
...
def save_model_bundle(
    ...
    """Serialize the trained baseline model and the metadata needed to reuse it."""
